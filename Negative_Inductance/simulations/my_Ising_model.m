...
for iter = 1 : numIters
    ...
    if mod(iter, captureInterval) == 0
        ...
        image = repmat((spin + 1) * 128, [1, 1, 3]);
        
        % Convert RGB image to indexed image
        [indexedImage, colormap] = rgb2ind(image, 256);
        
        % Save indexed image as a frame in the animation file
        if iter == captureInterval
            imwrite(indexedImage, colormap, outputFile, 'gif', 'LoopCount', Inf, 'DelayTime', 0.1);
        else
            imwrite(indexedImage, colormap, outputFile, 'gif', 'WriteMode', 'append', 'DelayTime', 0.1);
        end
    end
end
