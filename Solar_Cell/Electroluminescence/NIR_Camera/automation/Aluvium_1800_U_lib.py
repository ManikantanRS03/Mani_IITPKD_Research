...
def setup_camera(cam: Camera, exposure_us = 1000, gain_db = 10.0, frame_rate = 5):
    with cam:
        # === User-defined values ===
        #exposure_us       # in microseconds
        #gain_db           # in decibels
        #whitebalance_temp # in Kelvin (typical daylight)
        try:
            cam.ExposureAuto.set('Off')  # Turn off auto
            cam.ExposureTime.set(exposure_us)
            print(f"Exposure set to {exposure_us} µs")
        except (AttributeError, VmbFeatureError):
            print("Exposure setting not supported.")
        ...
        try:
            ...
            print("Gain setting not supported.")

        # === Frame rate ==
        try:
            cam.AcquisitionFrameRateEnable.set(True)
            cam.AcquisitionFrameRate.set(frame_rate)  # 5 fps → allows exposure up to ~200 ms
            print(f"Frame rate set to {frame_rate} fps")
        except (AttributeError, VmbFeatureError):
            print("Frame rate setting not supported.")




def setup_camera_continous(cam: Camera):
    with cam:
