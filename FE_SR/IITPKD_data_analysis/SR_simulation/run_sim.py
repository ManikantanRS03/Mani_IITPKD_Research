...
## Read the Parse file
params_path = 'parameter_IITPKD.txt'  # Parse input file
...
## Run the data generation simulation (simulating TDGL with experimental signal)
if(GEN_TDGL_EXP == 1):
    run = Simulation() #creating the object of the class simulation
    input_dir = "C:\\Drive\\FE_SR\\IITPKD_data_analysis\\cap_D\\SR\\freq_100"
    ens = 10
    sd = np.arange(0, 2, 0.1)
    run.SR_sim_with_FEdrive(params, input_dir, output_dir) #run simulation type
    # copy the parameters to the destination
    destination = os.path.join(output_dir, 'parameters.txt')
    shutil.copy2(params_path, destination)
...
if(ANL == 1):
    ...
    os.makedirs(os.path.dirname(output_pdf_path), exist_ok=True)
    with PdfPages(output_pdf_path) as pdf:
        sd_crt, bw_crt, Dext_delF, PSR, NF, SNR, COV = anl.SR_matrics(params, raw_data_file , pdf, delw =5 , delw_noise = 70)
    ...
    # Read the exp analysis from file Load and unpack columns
    # sd_crt_exp, PSR_exp, SNR_exp, COV_exp = np.loadtxt("C:\\Drive\\FE_SR\\IITPKD_data_analysis\\cap_D\\SR\\SR_matric_exp.txt", unpack=True, skiprows=1)
    
    # Analyse the experiment results 
    input_dir = "C:\\Drive\\FE_SR\\IITPKD_data_analysis\\cap_D\\SR\\freq_100"
    output_pdf_path = os.path.join(output_dir, "analysis", "all_PSD_plots_exp.pdf")
    os.makedirs(os.path.dirname(output_pdf_path), exist_ok=True)
    with PdfPages(output_pdf_path) as pdf:
        sd_crt_exp, bw_crt_exp, Dext_delF_exp, PSR_exp, NF_exp, SNR_exp, COV_exp = anl.SR_matric_exp(params, input_dir, pdf, delw = 5, delw_noise = 70)
    print("SR matric calculation done (experiment)! PSD plot generation done!")
    print("sd_cr = ", sd_crt_exp)
    print("bw_crt = ", bw_crt_exp)

    #generating SR matric plots overlayed with exp
    output_pdf_path = os.path.join(output_dir, "analysis", "SR_matric_theo_exp.pdf")
    os.makedirs(os.path.dirname(output_pdf_path), exist_ok=True)
    with PdfPages(output_pdf_path) as pdf:
         anl.SR_matric_plots_overlay_exp(pdf, sd_crt, Dext_delF_exp, PSR, SNR, COV, sd_crt_exp, Dext_delF_exp, PSR_exp, SNR_exp, COV_exp, sd_opt, Dext_delF_opt)
    print("SR matric plot generation done (experiment)!")